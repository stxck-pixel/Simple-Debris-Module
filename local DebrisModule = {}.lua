...
local function createDebrisChunk(origin, angleDeg, config, baseCFrame)
	...
	if directionToOrigin.Magnitude <= 0 then return end
	...
	local riseTween = TweenService:Create(
		debris,
		TweenInfo.new(config.SpawnTime, Enum.EasingStyle.Sine, Enum.EasingDirection.Out),
		{
			Transparency = 0,
	...
	task.delay(config.Lifetime, function()
		if not debris or not debris.Parent then return end
		local fade = TweenService:Create(debris, TweenInfo.new(config.FadeOutTime), { Transparency = 1 })
		fade:Play()
...
function DebrisModule:CreateDebris(originPosition, customConfig, baseCFrame)
	...
	for i = 1, config.DebrisCount do
		local angle = (360 / config.DebrisCount) * i + math.random(-10, 10)
		createDebrisChunk(originPosition, angle, config, baseCFrame)
	end
end
...
function DebrisModule:CreateWallDebris(originPosition, normalVector, customConfig)
	...
	if right.Magnitude <= 0 then return end
	right = right.Unit

	local up = right:Cross(normalVector)
	if up.Magnitude <= 0 then return end
	up = up.Unit

	for i = 1, config.DebrisCount do
		local angle = math.rad((360 / config.DebrisCount) * i + math.random(-10, 10))
		local radiusJitter = math.random(-3, 3)
		local distance = config.Radius + radiusJitter
		local tilt = math.rad(-math.random(20, 40))

		local offset = right * math.cos(angle) * distance + up * math.sin(angle) * distance
		...
		local baseCF = CFrame.fromMatrix(hitPos, rightAxis, upAxis)
		local arcRot = CFrame.Angles(tilt, 0, 0)

		debris.CFrame = baseCF * arcRot
		debris.Position = debris.Position - normalVector * (debris.Size.Z + 1)
		debris.Parent = visuals
		...
		local rise = TweenService:Create(
			...
			{
				Transparency = 0,
				Position = debris.Position + normalVector * (debris.Size.Z + 1)
			}
		)
		rise:Play()
...
function DebrisModule:CreateDoubleCircle(originPosition, customConfig, baseCFrame)
	...
	local function getGroundData(worldPos: Vector3)
		local rayOrigin = worldPos + Vector3.new(0, config.RaycastUp, 0)
		local rayDir = Vector3.new(0, -config.RaycastDown, 0)
	...
	local function createPolygonRing(radius: number, count: number, scaleJitter: number)
		...
		for i = 1, count do
			...
			local worldOffset = ringCF:VectorToWorldSpace(localOffset)
		...
		for i = 1, count do
			...
			local lengthScale = 1 + ((math.random() * 2 - 1) * scaleJitter)
			local heightScale = 1 + ((math.random() * 2 - 1) * scaleJitter)
			local thicknessScale = 1 + ((math.random() * 2 - 1) * scaleJitter)

			local part = createMatchingPartFromSource(current.SourcePart)
			...
			local baseCF = CFrame.lookAt(midpoint, midpoint + forward, up)

			local inwardTilt = math.rad(math.random(20, 40))
			local rollJitter = math.rad(math.random(-config.RotationJitter * 100, config.RotationJitter * 100) / 100)

			part.CFrame = baseCF * CFrame.Angles(0, 0, inwardTilt + rollJitter)

			part.Position -= up * (config.SpawnDepth + part.Size.Y * 0.5)
			part.Parent = visuals
			...
			local riseTween = TweenService:Create(
				...
				{
					Transparency = 0,
					Position = part.Position + up * (config.SpawnDepth + part.Size.Y * 0.5)
				}
			)
			riseTween:Play()
	...
	createPolygonRing(config.InnerRadius, config.InnerCount, config.InnerScaleJitter)
	createPolygonRing(config.OuterRadius, config.OuterCount, config.OuterScaleJitter)
end
